...
class SalesAndReportsCollection
    ...
    def rows
        ...
        items_grouped_by_brand =  @items.filter { |item| item.product_type_identifier == "1F" }.group_by { |item| item.sku }
        items_grouped_by_brand.map { |brand, items| 
